...
def main() -> None:
    ...
    for target_arch, torch_versions in ARCH_TORCH_PAIRS.items():
        for torch_version in torch_versions:
            ...
            # Build for all supported Python versions since DeepGEMM doesn't use stable ABI.
            python_versions = TORCH_PYTHON_SUPPORT[torch_x_y]

            cuda_versions = PYTORCH_CUDA_VERSIONS[(torch_x_y, target_arch)]
            for cuda_version in cuda_versions:
                cuda_version_parsed = Version(cuda_version)

                # DeepGEMM requires CUDA 12.3+ (we use 12.8+ for best compatibility)
